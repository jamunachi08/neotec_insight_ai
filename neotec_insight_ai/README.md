...
## Architecture (v2.1 — shell + embed)

This app is a **shell + AI layer**, not a second reporting engine. It does **not**
duplicate report tables. Instead:

- **Reports / authoring** come from the main **Neotec Insight** app. The Desk page
  at `/app/insight` embeds that app's real SPA (`/insight`) in an iframe — so every
  view (Period · Dimension · Years · Combo · Trial Balance · Balance Sheet · CFO
  Briefing · Dashboard · Group · Visuals) and all authoring (Run · Equity · Rows ·
  Account map · Budget) is the genuine, user-built report — same backend, same
  numbers. Users create rows and bind chart-of-accounts + dimensions there.
- **Neotec AI** is this app's value-add: a slide-over panel that reads the *live*
  numbers from the selected report (server-side, via the main app's engine) and
  answers through NeoNexus (OpenAI-compatible, local-first). Bilingual EN/AR.

The report LIST and the AI context are sourced from the main app:
`get_bootstrap` reads `Insight Report Definition`; `ask_ai` calls the engine
(`run_trial_balance` / `run_balance_sheet` / `run_report`) for the chosen report and
posting-date window, then sends a trimmed JSON of the real figures to the model.

### AI setup — all bundled in this app
Everything needed for AI ships here, no extra files:
- **Insight Settings** (Single DocType): `ai_enabled`, `ai_endpoint`
  (default `http://host.docker.internal:11434/v1`), `ai_model` (default `llama3.3`),
  `ai_api_key`, `ai_system_prompt`, `language_default` — seeded on install.
- **Whitelisted methods**: `get_bootstrap`, `run_report`, `ask_ai`, `list_models`.
- **Translations**: `public/insight/i18n.ar.json`, `translations/ar.csv`.

Point `ai_endpoint` at NeoNexus / Ollama / LM Studio / OpenAI, set `ai_model`,
tick `ai_enabled`, and the panel goes live.
